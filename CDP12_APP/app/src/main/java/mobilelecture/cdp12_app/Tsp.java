...
public class Tsp {
    ...
    // new add
    public void readStringPath(String path) {

        String line[] = path.split("\n");
        int i = 0;

        while ( i < line.length) {
            String splitA[] = line[i].split(" ");
            LinkedList<String> split = new LinkedList<String>();
            for (String s : splitA)
                if (!s.isEmpty())
                    split.add(s);
    ...
    private int selectNextTown(Ant ant) {
        ...
        Log.i("TSP :","Not supposed to get here.");
        return 0;
    ...
    public int[] solve(int input_i) {
        ...
        if (input_i > 3 && bestTour[0] == 0) {
            //System.out.println("Best tour length: " + (bestTourLength - n));
            //System.out.println("Best tour:" + tourToString(bestTour));
            Log.i("TSP : ", "" + "Best tour length: " + (bestTourLength - n));
            Log.i("TSP : ", "" + "Best tour:" + tourToString(bestTour));
        }
